...
MODULE_PARAMETER_SPECS: dict[str, tuple[ParameterSpec, ...]] = {
    ...
    "primitive": (
        ParameterSpec(
            "kind", "스텝 종류", "choice", G_METHOD, "kind",
            default="rest",
            choices=(
                Choice("rest", "휴지 (Rest)", "전류를 걸지 않고 지정한 시간만큼 둡니다."),
                Choice("ocv", "개방전압 (OCV)", "개방 회로 전압을 지정한 시간 동안 측정합니다."),
                Choice("cc_charge", "정전류 충전 (CC)", "일정 전류로 종료 전압까지 충전합니다."),
                Choice("cccv_charge", "정전류·정전압 충전 (CCCV)",
                       "목표 전압까지 정전류로 올린 뒤 전류가 기준 아래로 떨어질 때까지 유지합니다."),
                Choice("cv_charge", "정전압 충전 (CV)", "목표 전압을 지정한 시간 동안 유지합니다."),
                Choice("cc_discharge", "정전류 방전 (CC)", "일정 전류로 종료 전압까지 방전합니다."),
            ),
            help="이 스텝이 무엇을 하는지 고릅니다. 선택에 따라 아래 항목이 바뀝니다.",
            basis="modules/primitive.py 의 PRIMITIVE_KINDS",
            affects="스텝 유형과 종료 조건",
            risk="caution", verification="software-checked",
        ),
        _c_rate(
        ...
        ParameterSpec(
            ...
            default=4.2, minimum=2.0, maximum=5.0,
            help="정전압 구간에서 유지할 전압입니다.",
            basis="셀 상한 전압", affects="CCCV·CV 스텝의 목표 전압",
            visible_when=(("kind", ("cccv_charge", "cv_charge")),),
            risk="critical", verification="software-checked",
        ),
        ParameterSpec(
            "charge_end_voltage_v", "충전 종료 전압", "voltage_v", G_CHARGE,
            "charge_end_voltage_v",
            default=4.2, minimum=2.0, maximum=5.0,
            help="이 전압에 도달하면 충전을 멈춥니다.",
            basis="셀 상한 전압", affects="CC 충전 스텝의 종료 조건",
            visible_when=(("kind", ("cc_charge",)),),
            risk="critical", verification="software-checked",
        ),
        ParameterSpec(
            "discharge_end_voltage_v", "방전 종료 전압", "voltage_v", G_DISCHARGE,
            "discharge_end_voltage_v",
            default=2.5, minimum=2.0, maximum=5.0,
            help="이 전압까지 내려가면 방전을 멈춥니다.",
            basis="셀 하한 전압", affects="CC 방전 스텝의 종료 조건",
            visible_when=(("kind", ("cc_discharge",)),),
            risk="critical", verification="software-checked",
        ),
        _c_rate(
            "cv_cutoff_c_rate", "정전압 종료 전류", 0.05, group=G_CHARGE,
            basis="랩 표준 0.05C", affects="정전압 구간의 종료 조건",
            recommended_max=0.5,
            visible_when=(("kind", ("cccv_charge", "cv_charge")),),
        ),
        _rest(
        ...
        ParameterSpec(
            ...
            default=1, minimum=1.0, maximum=10000.0, recommended_max=1000.0,
            help="2 이상이면 이 스텝만 그 횟수만큼 반복합니다. "
                 "반복 구간은 이 구간 안에서 닫히므로 순서를 바꿔도 안전합니다.",
            basis="composer 의 LOOP 계약 — 대상은 같은 구간 안이어야 합니다.",
            affects="스텝 수 (반복 시 LOOP 스텝이 하나 추가됩니다)",
            verification="software-checked",
        ),
    ),
    "custom_steps": (
