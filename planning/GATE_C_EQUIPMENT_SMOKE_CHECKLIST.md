...
| 저장 실패(Cycle/Loop) 없이 재저장된다 | ☐ | 이전 실패 원인 해소 확인 |
| 공통 안전조건 용량이 표시된다 (0 아님) | ☐ | PNE02 기대: **800 mAh** (`0x3D8+12` / `0x458+12`) |
| 충전 스텝 값이 표 대로 표시된다 | ☐ | 전류/전압/cutoff/샘플링 |
| 방전 스텝 값이 표 대로 표시된다 | ☐ | 전류/종료전압/샘플링 |
| DOD/SOC 40%가 표시되는가 (있다면) | ☐ | 선택 |
| LOOP count=2, goto=step1이 표시된다 | ☐ | |
| END 스텝이 있다 | ☐ | |
