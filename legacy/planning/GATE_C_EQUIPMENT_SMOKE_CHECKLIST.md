...
상태: **통과 (PNE02, 2026-09-08)** — 결과표 서명 완료. Gate C exit.
...
| 출력 파일 | `example/smoke_writer_probe.sch` — 사전 생성됨, 6스텝 (Cycle 마커 없음) |
...
1. 사전 생성된 파일 사용을 권장합니다: `example/smoke_writer_probe.sch`
   ...
   ```
2. `smoke_writer_probe.sch`를 PNE PC로 복사합니다. **(주의)** 이전 파일이
   이미 채널에 열려 있었다면, PNE PC의 옛 파일을 새 파일로 덮어쓴 뒤 다시 여세요.
3. **CTSEditorPro**에서 엽니다 (1차: 저장만 수행; **셀에서 실행하지 마세요**).
4. UI 값을 스텝별로 확인합니다 (아래 표의 "기대값"과 정확히 일치해야 함):

| 스텝 | 종류 | 확인 항목 | 기대값 |
|------|------|-----------|--------|
| 공통 | 시험/공통 안전조건 | 용량 (0/.000 이면 안 됨) | **800 mAh** (셀 80의 10× 상한) — PNE02 기준 |
| 공통 | 시험/공통 안전조건 | Vmax / Vmin / 온도 | 4.200 V / 2.500 V / 70 |
| 공통 | 시험/공통 안전조건 | 최대 전류 | **800 mA** |

> **PNE02 vs PNE16:** 용량 오프셋이 다릅니다. C5 기준 장비는 **PNE02**.  
> PNE02는 `0x3D8+12` / `0x458+12`에 용량을 둡니다. PNE16에서 `+16`에 넣으면 최소 전류로 오인됩니다.
| 1 | CCCV 충전 | 전류 / 전압 / CV cutoff | **8 mA (0.008 A)** / **4.200 V** / **4 mA** |
| 1 | CCCV 충전 | 샘플링 간격 / ΔV | **30 s** / **20 mV** |
| 2 | Rest | 시간 / 샘플링 간격 | **50 s** / **25 s** |
| 3 | CC 방전 | 전류 / 종료전압 | **8 mA** / **2.500 V** |
| 3 | CC 방전 | 전압 제한(V-limit) 표시값 | **≈2.000 V — 정상** (cutoff 2.5V와 다른 필드) |
| 3 | CC 방전 | 샘플링 간격 / ΔV / (있다면) SOC·DOD | **15 s** / **5 mV** / (선택) 40% |
| 4 | Rest | 시간 / 샘플링 간격 | **35 s** / **10 s** |
| 5 | LOOP | count / goto 대상 | **2** / **step 1 (충전으로 복귀)** |
| 6 | END | 존재 여부 | 존재 |

> **Cycle 마커:** CTS 저장 규칙상 Cycle은 Loop로 닫혀야 합니다. PNE02에서 검증된
> 루프 파일은 Cycle 없이 `충전→…→Loop→END` 구조라, 이 프로브도 동일합니다.
> (이전 버전은 Cycle을 넣어 "Loop 종료 없이 새 Cycle" 저장 실패가 났습니다.)
...
| 확인 항목 | 통과? | 비고 |
|-------|:-----:|-------|
| CTSEditorPro가 오류 없이 파일을 연다 | 통과 | |
| 저장 실패(Cycle/Loop) 없이 재저장된다 | 통과 | 이전 실패 원인 해소 확인 |
| 공통 안전조건 용량이 표시된다 (0 아님) | 통과 | PNE02 기대: **800 mAh** (`0x3D8+12` / `0x458+12`) |
| 충전 스텝 값이 표 대로 표시된다 | 통과 | 전류/전압/cutoff/샘플링 |
| 방전 스텝 값이 표 대로 표시된다 | 통과 | 전류/종료전압/샘플링 |
| DOD/SOC 40%가 표시되는가 (있다면) | 확인 못함 / UI 미표시 | 선택 — Gate C 비차단 |
| LOOP count=2, goto=step1이 표시된다 | 통과 | |
| END 스텝이 있다 | 통과 | |
| 재저장 시 무관한 필드가 깨지지 않는다 | 통과 | `compare_sch` JSON 첨부 |
| (선택) 채널 실행이 시작/진행된다 | 통과 | |
