...
# -*- coding: utf-8 -*-
"""
sch_writer_fixed.py -- PNE CTSeditorPro .sch 바이너리 생성기 (수정 버전)
JSON 중간 표현 -> .sch 파일 변환
"""
...
def insert_auto_rest(steps, test_type, rest_record_time='30s'):
    """Insert automatic rest after charge/discharge steps based on test_type"""
...
def parse_duration(s, capacity_mA=None):
    """Parse duration strings: 1.5C, 30s, 2min, 3h, 2d"""
    if s is None:
        return 0.0
    s = str(s).strip()
    if s.upper().endswith('C'):
        if capacity_mA is None:
            raise ValueError("C-rate needs cell_capacity_mAh")
...
def validate_step(step, capacity_mA):
    ...
    if missing:
        raise ValueError("Step '%s' missing: %s" % (stype, missing))
...
def build_step_blocks(schedule_json):
    """Build step blocks from JSON schedule"""
    ...
    followup = schedule_json.get('followup', None)
    ...
    if test_type:
        ...
        if cycles:
            cycles = [
                {**cyc, 'steps': insert_auto_rest(cyc.get('steps', []), test_type)}
                for cyc in cycles
            ]
        if followup:
            f_loop = followup.get('loop', {})
            if 'steps' in f_loop:
                f_loop = dict(f_loop)
                f_loop['steps'] = insert_auto_rest(f_loop['steps'], test_type)
            followup = {**followup, 'loop': f_loop}

    if test_type == 'ratetest_cyclelife' and cycles is not None:
        return _build_ratetest_cyclelife(capacity, cycles, followup)
...
def _build_ratetest_cyclelife(capacity, cycles, followup):
    """Rate Test + CycleLife composite structure"""
    blocks = []
    idx = 1

    # Rate Test cycles (first cycle has flag_offset=496, rest have 64)
    ...
    # Followup CycleLife section
    if followup:
        pre_steps = followup.get('pre_loop', [])
        loop_def = followup.get('loop', {})
        ...
        reset_cap = loop_def.get('reset_capacity', True)
...
def _build_single_loop(capacity, pre_steps, loop_def):
    """Single loop structure (CycleLife, Formation)"""
    blocks = []
    idx = 1
...
def _build_multi_cycle(capacity, pre_steps, cycles):
    """Multi-cycle structure (Rate Test)"""
    blocks = []
    idx = 1
...
def json_to_sch(json_input, output_path):
    """Convert JSON schedule to .sch binary file"""
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print("Usage: python sch_writer_fixed.py <input.json> <output.sch>")
        sys.exit(1)
